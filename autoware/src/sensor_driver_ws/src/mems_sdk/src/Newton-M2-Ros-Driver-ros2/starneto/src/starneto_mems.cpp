...
namespace ns_starneto_mems {
    ...
    void Starneto::readSerial() {
        try {
            ...
            RCLCPP_ERROR(nodeHandle->get_logger(), "Port crashed! Please check cable!");
            ...
            if (protocolFlag == UNKNOWN_PROTOCOL) {
                ...
                if (select_flag && rbuf[i] == ',') {
                    if (strncmp(OneFrame, "$GPFPD,", 7) == 0) {
                        ...
                        protocolFlag = UNKNOWN_PROTOCOL;
                        RCLCPP_ERROR(nodeHandle->get_logger(), "UNKNOWN_PROTOCOL.");
                ...
                switch (GPFPD_STATE_PARSER) {
                    ...
                    case 3: //等待接收状态字节，第一个字符
                        ...
                        if ((rbuf[i] >= '0' && rbuf[i] <= '9') || (rbuf[i] >= 'A' && rbuf[i] <= 'F'))//状态字节应是一个十六进制数
                        {
                            ...
                            RCLCPP_ERROR(nodeHandle->get_logger(), "DATA ERROR IN CASE3.");
                    ...
                    case 4: //等待接收状态字节，第二个字符
                        ...
                        if ((rbuf[i] >= '0' && rbuf[i] <= '9') || (rbuf[i] >= 'A' && rbuf[i] <= 'F'))//状态字节应是一个十六进制数
                        {
                            ...
                            memset(OneFrame, 0, sizeof(OneFrame));
                            GPFPD_STATE_PARSER = 0;
                            RCLCPP_ERROR(nodeHandle->get_logger(), "DATA ERROR IN CASE4.");
                    ...
                    case 5: //等待校验和标志*
                        ...
                        if (rbuf[i] == '*') {
                            ...
                            memset(OneFrame, 0, sizeof(OneFrame));
                            GPFPD_STATE_PARSER = 0;
                            RCLCPP_ERROR(nodeHandle->get_logger(), "DATA ERROR IN CASE5.");
                        }
                        break;
                    case 6: //校验和第一个字符
                        OneFrame[CntByte] = rbuf[i];
                        ...
                        if ((rbuf[i] >= '0' && rbuf[i] <= '9') || (rbuf[i] >= 'A' && rbuf[i] <= 'F'))//校验和字节应是一个十六进制数
                        {
                            ...
                            memset(OneFrame, 0, sizeof(OneFrame));
                            GPFPD_STATE_PARSER = 0;
                            RCLCPP_ERROR(nodeHandle->get_logger(), "DATA ERROR IN CASE6.");
                        }
                        break;
                    case 7: //校验和第二个字符
                        OneFrame[CntByte] = rbuf[i];
                        ...
                        if ((rbuf[i] >= '0' && rbuf[i] <= '9') || (rbuf[i] >= 'A' && rbuf[i] <= 'F'))//校验和字节应是一个十六进制数
                        {
                            ...
                            if (cscomputed != csreceived)//校验和不匹配
                            {
                                memset(OneFrame, 0, sizeof(OneFrame));
                                GPFPD_STATE_PARSER = 0;
                                RCLCPP_ERROR(nodeHandle->get_logger(), "DATA ERROR IN CASE7.");
                            } else//校验和匹配
                            {
                    ...
                    case 8: //等待结束标志<CR>=0x0d
                        ...
                        if (rbuf[i] == '\r') {
                            ...
                            memset(OneFrame, 0, sizeof(OneFrame));
                            GPFPD_STATE_PARSER = 0;
                            RCLCPP_ERROR(nodeHandle->get_logger(), "DATA ERROR IN CASE8.");
                    ...
                    case 9: //等待结束标志<LF>=0x0a
                        ...
                        if (rbuf[i] == '\n') {
                            ...
                            RCLCPP_INFO(nodeHandle->get_logger(), "GPS DATA OK.");
                            ...
                            RCLCPP_ERROR(nodeHandle->get_logger(), "DATA ERROR IN CASE9.");
                        }
                        memset(OneFrame, 0, sizeof(OneFrame));
                        GPFPD_STATE_PARSER = 0;
                ...
                switch (GTIMU_STATE_PARSER) {
                    ...
                    case 3: //校验和第一个字符
                        ...
                        if ((rbuf[i] >= '0' && rbuf[i] <= '9') || (rbuf[i] >= 'A' && rbuf[i] <= 'F'))//校验和字节应是一个十六进制数
                        {
                            ...
                            RCLCPP_ERROR(nodeHandle->get_logger(), "IMU ERROR IN CASE3.");
                        }
                        break;
                    case 4: //校验和第二个字符
                        OneFrame[CntByte] = rbuf[i];
                        ...
                        if ((rbuf[i] >= '0' && rbuf[i] <= '9') || (rbuf[i] >= 'A' && rbuf[i] <= 'F'))//校验和字节应是一个十六进制数
                        {
                            ...
                            if (cscomputed != csreceived)//校验和不匹配
                            {
                                memset(OneFrame, 0, sizeof(OneFrame));
                                GTIMU_STATE_PARSER = 0;
                                RCLCPP_ERROR(nodeHandle->get_logger(), "IMU ERROR IN CASE4.");
                            } else//校验和匹配
                            {
                    ...
                    case 6: //等待结束标志<LF>=0x0a
                        ...
                        if (rbuf[i] == '\n') {
                            //printf("A good frame!\n");
                            RCLCPP_INFO(nodeHandle->get_logger(), "IMU DATA OK");
